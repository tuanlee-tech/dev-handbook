...
/**
 * Clamp value vào khoảng cho phép
 * WHY:
 * - Đảm bảo state luôn hợp lệ
 * - Dùng chung cho mọi action
 */
export function clampCount(value: number): number {
  return Math.min(MAX_COUNT, Math.max(MIN_COUNT, value));
}

export default function AdvancedCounter(): JSX.Element {
  /**
   * ================================
   * STATE MODEL (Undo / Redo Pattern)
   * ================================
   * past    : undo stack
  ...
  const [present, setPresent] = useState<number>(INITIAL_COUNT);
   ...
   * CORE HELPER
   * ================================
   * Apply một giá trị mới cho counter
   *
   * RULE:
   * - Mọi action mới đều clear redo stack
   */
  const applyNewValue = (nextValue: number): void => {
    setPast(prevPast => [...prevPast, present]); // save current to undo stack
    setPresent(nextValue);                       // update current value
    setFuture([]);                               // invalidate redo history
  };
  ...
  const handleIncrement = (): void => {
    applyNewValue(clampCount(present + step));
  };
   ...
   *
   * EDGE CASE:
   * - Không undo nếu past rỗng
   */
  const handleUndo = (): void => {
    if (!canUndo) return;

    const previousValue: number = past[past.length - 1];
   ...
   *
   * EDGE CASE:
   * - Không redo nếu future rỗng
   */
  const handleRedo = (): void => {
    if (!canRedo) return;

    const nextValue: number = future[0];
   ...
   *
   * WHY:
   * - Reset phải clear toàn bộ timeline
   */
  const handleReset = (): void => {
    setPast([]);
    setPresent(INITIAL_COUNT);
    setFuture([]);
  };
  ...
  return (
    <div className="counter">
      <h2 className="counter__title">Advanced Counter</h2>

      {/* STEP CONTROL */}
      <div className="counter__step">
        <label
          htmlFor="counter-step-input"
          className="counter__step-label"
        >
          Step
        </label>
      ...
      <div className="counter__actions">
        <button
          ...
          onClick={handleUndo}
          disabled={!canUndo}
          className="counter__action counter__action--undo"
        >
          Undo
        </button>
        ...
        <button
          ...
          onClick={handleRedo}
          disabled={!canRedo}
          className="counter__action counter__action--redo"
        >
          Redo
        </button>
        ...
        <button
          type="button"
          onClick={handleReset}
          className="counter__action counter__action--reset"
        >
          Reset
        </button>
      </div>
...
/**
 * ================================
 * COMPONENT TEST: AdvancedCounter
 * ================================
 * WHY:
 * - Test behavior của component
 * - Không test implementation details
 * - Không test CSS
 */

describe('AdvancedCounter component', () => {
    beforeEach(() => {
        render(<AdvancedCounter />);
    });

    it('should render initial count', () => {
        // WHAT: giá trị ban đầu phải là 0
        expect(screen.getByTestId('count-value')).toHaveTextContent('0');
    });

    it('should increment and decrement correctly', async () => {
    ...
    it('should disable undo and redo buttons correctly', async () => {
        const user = userEvent.setup();
...
/* =======================
   Utils
======================= */
// WHY: isolate creation logic → dễ test, dễ thay đổi default
...
export default function TodoApp() {
  ...
  /* ---------- Effects ---------- */
  // Sync todos → localStorage on every mutation
  useEffect(() => {
  ...
  const toggleAll = () => {
    // WHY: derived state → không lưu state phụ
