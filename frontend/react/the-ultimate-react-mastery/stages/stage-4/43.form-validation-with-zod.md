...
const translations = {
  ...
  vi: {
    ...
    capacityExceeded:
      'Tổng số người tham dự không được vượt quá sức chứa sự kiện',
...
function SuccessScreen({
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      ...
      >
        <h2>
          {language === 'en' ? 'Registration Details' : 'Chi tiết đăng ký'}
        </h2>
...
// latest request only
// Đảm bảo kết quả trả về tránh race condition
/*
let lastCallId = 0;
...
validate: async (value) => {
  ...
  if (callId !== lastCallId) return true;

  return result.available || 'Username taken';
}
*/

//Nếu muốn “clean architecture” hơn
/*
UI Form
  ↓
Zod (sync validation)
  ↓
async validate (API check)
  ↓
submit
*/
const usernameSchema = z.string().min(3);
...
async function validateUsername(value) {
  usernameSchema.parse(value); // sync first
...
6. **Q:** Architect schema structure cho large application với 50+ forms?
   ...
   **A:**
   - Base schemas trong `/schemas/base/`
     - Chứa các primitive schemas dùng lại nhiều nơi như email, phone, id, string rules
     - Không chứa business logic, chỉ validation thuần
     - Ví dụ:
   ...
   - Domain-specific schemas trong `/schemas/domains/`
     - Đại diện cho entity nghiệp vụ (User, Event, Order…)
     - Là source of truth cho data model của hệ thống
     - Có thể reuse base schemas
     - Ví dụ:
   ...
   - Shared validation rules trong `/schemas/rules/`
     - Chứa business rules dùng chung (minAge, futureDate, passwordStrength…)
     - Dùng `.refine()` hoặc `.superRefine()` cho logic phức tạp
     - Ví dụ:

       ```ts
       export const minAge = z.number().min(18);
       ```

   - Type exports từ schemas
     - Luôn derive type từ schema để đảm bảo single source of truth
     - Tránh viết interface thủ công gây mismatch FE/BE
     - Ví dụ:
   ...
   - Schema composition patterns
     - `.extend()` dùng cho inheritance (mở rộng schema theo domain)
     - `.merge()` dùng để combine 2 schema độc lập
     - `.pick()` / `.omit()` dùng để tạo view-specific schema (login, public API…)
     - Forms chỉ là layer composition, không phải source of truth
     - Ví dụ form:
...
"Team có 30 forms với inline validation duplicate. Mất 2 tuần refactor sang Zod schemas. Kết quả: giảm 40% validation code, zero regression bugs (không có lỗi phát sinh lại từ những chức năng đã từng chạy đúng trước đó) vì tests schemas riêng. Thêm feature mới giờ chỉ mất 1/3 thời gian."
