...
function ModalProvider({ children }) {
  ...
  const openModal = useCallback((component, props = {}) => {
    ...
    // Tách callbacks ra khỏi props, lưu vào ref
    const callbacks = {};
    const plainProps = { ...props };
    Object.keys(props).forEach((key) => {
      if (typeof props[key] === 'function') {
        callbacks[key] = props[key];
        delete plainProps[key];
      }
    });
  ...
  const closeModal = useCallback(() => {
    setModals((prev) => {
      ...
      callbacksRef.current.delete(removedId);
  ...
  const getCallbacks = useCallback((id) => {
...
function ModalPortal() {
  const { activeModal, closeModal, getCallbacks } = useModal();

  if (!activeModal) return null;

  const { id, component: ModalComponent, props } = activeModal;
...
function DemoPage() {
  ...
  const handleStackModals = () => {
    openModal(ConfirmModal, {
      ...
      onConfirm: () => {
        console.log('Modal 1: mở Modal 2');
        openModal(ConfirmModal, {
          title: 'Modal 2',
          message: 'Đây là modal thứ hai trong stack!',
          onConfirm: () => {
            console.log('Modal 2: mở Modal 3');
            openModal(DetailModal, {
              title: 'Modal 3 - Final',
              content:
                'Stack hoạt động đúng! Nhấn ESC hoặc click overlay để đóng từng cái một.',
            });
          },
    ...
    <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      >
        ...
        <ul>
          ...
          <li>✅ closeModal() — đóng modal trên cùng</li>
          ...
          <li>✅ Multiple modals stack (LIFO)</li>
        </ul>
      </div>
