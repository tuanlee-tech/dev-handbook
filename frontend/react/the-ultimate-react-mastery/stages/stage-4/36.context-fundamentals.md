...
function ModalPortal() {
  const { activeModal, closeModal, closeAllModals } = useModal();
...
// ✅ Nút confirm chỉ gọi onConfirm, KHÔNG gọi onClose
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
        ...
        <button
          onClick={() => {
            ...
            // Nếu gọi, modal mới vừa push vào stack sẽ bị pop ngay lập tức
          }}
          style={{
            padding: '10px 20px',
            border: 'none',
...
// ✅ Step cuối: có thể đóng cả stack bằng onCloseAll
function DetailModal({ title, content, onClose, onCloseAll }) {
  return (
...
function DemoPage() {
  ...
  const handleViewDetail = () => {
    openModal(DetailModal, {
      ...
      content: 'Đây là mô tả chi tiết về sản phẩm...',
    });
  };
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <p>
        Stack size: <strong>{modals.length}</strong>
      </p>
      ...
      >
        <h3>📝 Features:</h3>
        <ul>
          <li>✅ openModal(component, props)</li>
          <li>✅ closeModal() — đóng modal trên cùng (LIFO)</li>
          <li>✅ closeAllModals() — xóa toàn bộ stack</li>
...
> **Context API** là cơ chế "phát sóng" dữ liệu của React, cho phép component chia sẻ state xuyên suốt cây component mà không cần truyền props qua từng tầng.
>
> **Giải quyết:** Vấn đề **Props Drilling** — khi data cần đi qua 3–5+ tầng component trung gian (không dùng đến data) chỉ để đến component lá cần nó. Làm code khó maintain, khó refactor, component bị couple vào props không liên quan.
>
> ```jsx
> // Không cần: App → Layout → Header → Nav → ThemeToggle (truyền theme qua 4 tầng)
> // Chỉ cần: App (Provider) → ... → ThemeToggle (useContext)
> ```

2. Props drilling là gì? Tại sao là vấn đề?

> **Props drilling** là hiện tượng truyền props qua nhiều tầng component trung gian chỉ để đưa data đến component sâu bên dưới.
>
> **Vấn đề:**
>
> - **Khó maintain:** Thêm/sửa 1 prop phải sửa nhiều file
> - **Component bị couple:** Layout, Header phải nhận `theme` dù không dùng
> - **Khó reuse:** Component bị ràng buộc vào props cụ thể, khó dùng lại ở chỗ khác
> - **Dễ bug:** Quên truyền props ở tầng nào đó → `undefined`

3. useContext hook dùng như thế nào?

> 3 bước:
>
> 1. **Tạo Context** (ngoài component): `const ThemeContext = createContext(defaultValue)`
> 2. **Cung cấp value** (Provider): `<ThemeContext.Provider value={...}><App /></ThemeContext.Provider>`
> 3. **Tiêu thụ** (Consumer): `const { theme } = useContext(ThemeContext)`
>
> ```jsx
> const ThemeContext = createContext();
>
...
> ```
>
> **Lưu ý:** `useContext` chỉ tìm Provider gần nhất trong cây component. Nếu không có Provider → dùng `defaultValue` (nếu có) hoặc `undefined`.
...
1. So sánh Props drilling vs Context. Khi nào dùng cái nào?
   ...
   > | Tiêu chí         | Props Drilling        | Context API                          |
   > | ---------------- | --------------------- | ------------------------------------ |
   > | **Khoảng cách**  | 1–2 tầng              | 3+ tầng hoặc global                  |
   > | **Tính rõ ràng** | ✅ Explicit, dễ trace | ❌ Ẩn, khó biết data từ đâu          |
   > | **Reusability**  | ✅ Component độc lập  | ⚠️ Phụ thuộc Provider                |
   > | **Performance**  | ✅ Tối ưu mặc định    | ⚠️ Có thể gây re-render nếu dùng sai |
   > | **Maintain**     | ❌ Khó khi nhiều tầng | ✅ Dễ thêm/sửa data                  |
   >
   > **Quy tắc:**
   >
   > - **1–2 levels** → Props (đơn giản, explicit)
   > - **Global concerns** (theme, auth, language) → Context
   > - **Business logic phức tạp, thay đổi liên tục** → Không dùng Context (dùng Redux/Zustand/React Query)
...
2. Làm sao handle missing Provider?
   ...
   >
   > ```jsx
   > const ThemeContext = createContext({ theme: 'light' }); // fallback
   > ```
   >
   > **b) Custom hook throw error:**
   >
   > ```jsx
   > function useTheme() {
   >   const context = useContext(ThemeContext);
   ...
   > ```
   >
   > → Giúp phát hiện lỗi sớm ở development, không để crash khó hiểu ở production.
   >
   > **c) ESLint/TypeScript:** Bắt buộc component dùng hook phải nằm trong Provider.

3. Tại sao nên tạo custom hook cho Context?

> **a) Ẩn implementation detail:** Consumer không cần biết tên Context là gì.
>
> **b) Bắt lỗi runtime:** Throw error nếu dùng ngoài Provider.
>
> **c) Dễ refactor:** Đổi từ Context sang Zustand/Redux sau này chỉ sửa 1 file hook, không đụng 20 component.
>
> **d) Auto-complete & TypeScript:** Hook có type rõ ràng, không cần destructuring lung tung.

```jsx
// ❌ Mỗi chỗ dùng lại viết lại
const { theme } = useContext(ThemeContext);

// ✅ Một chỗ định nghĩa, mọi chỗ dùng
const { theme } = useTheme();
```
...
> **Vấn đề:** Khi Context value thay đổi, **TẤT CẢ** consumers đều re-render, dù chỉ cần 1 phần nhỏ trong value.
>
> **Nguyên nhân chính:**
>
> - Tạo object mới mỗi render: `value={{ theme, setTheme }}` → reference mới → trigger re-render
> - Nhét quá nhiều state không liên quan vào 1 Context
>
> **Optimize:**
>
> **a) Tách Context:** Mỗi concern 1 Context riêng
>
> ```jsx
> <ThemeProvider>
>   <AuthProvider>
...
> ```
>
> → Component chỉ subscribe Context nó cần.
>
> **b) `useMemo` cho value object:**
>
> ```jsx
> const value = useMemo(() => ({ theme, setTheme }), [theme]);
> ```
>
> **c) Tách state và dispatch:** (Pattern Ngày 37)
>
> ```jsx
...
>
> **d) `React.memo` + selectors:** Dùng thư viện như `use-context-selector` hoặc chuyển Zustand.

2. So sánh Context API vs Redux/Zustand

> |                 | Context API                  | Redux                             | Zustand                            |
> | --------------- | ---------------------------- | --------------------------------- | ---------------------------------- |
> | **Bản chất**    | Transport mechanism          | State management library          | State management library           |
> | **Boilerplate** | Ít                           | Nhiều                             | Rất ít                             |
> | **DevTools**    | ❌ Không                     | ✅ Redux DevTools, time-travel    | ✅ Redux DevTools                  |
> | **Performance** | ⚠️ Manual optimize           | ✅ Auto-optimized (selectors)     | ✅ Auto-optimized                  |
> | **Re-render**   | Tất cả consumers             | Chỉ component subscribe state đổi | Chỉ component subscribe state đổi  |
> | **Use case**    | Theme, auth, config (ít đổi) | Complex app, nhiều logic          | Simple–medium app, cần performance |
> | **Async logic** | Tự viết (useEffect)          | Redux Thunk/Saga                  | Tự viết hoặc middleware            |

> **Rule of thumb:**
>
> - Context = **transport** (đưa data đi)
> - Redux/Zustand = **state management** (quản lý state phức tạp, cache server data)

3. Khi nào KHÔNG nên dùng Context?

> **a) Data thay đổi liên tục (high-frequency updates):**
> Form input, real-time chart, animation → Context gây re-render cascade. Dùng local state hoặc atom state (Zustand/Jotai).
>
> **b) Server state (API data):**
>
> ```jsx
> // ❌ Anti-pattern
> <APIDataProvider>
> // → Dùng React Query / SWR (caching, refetch, stale-while-revalidate có sẵn)
> ```
>
> **c) Business logic phức tạp:**
> Nhiều interdependent state → dễ thành "spaghetti context". Dùng Redux/Zustand có devtools để debug.
>
> **d) Chỉ cần 1–2 levels props:**
> Props drilling rõ ràng hơn, dễ trace hơn Context ẩn.
>
> **e) Cần time-travel debugging / middleware:**
> Context không có devtools chính thức.

4. Multiple contexts vs single context - trade-offs?

> **Single Context (AppContext chứa mọi thứ):**
>
> - ✅ Ít boilerplate, dễ setup ban đầu
> - ❌ **Re-render nightmare:** Mỗi lần 1 field đổi → toàn bộ app re-render
> - ❌ Khó tree-shake, khó test riêng từng phần
> - ❌ Khó thay thế 1 phần (phải refactor cả AppContext)
>
> **Multiple Contexts (ThemeContext, AuthContext, CartContext...):**
>
> - ✅ **Separation of concerns:** Mỗi context 1 nhiệm vụ
> - ✅ **Performance:** Component chỉ re-render khi context nó dùng đổi
> - ✅ **Reusability:** ThemeContext có thể copy sang project khác
> - ✅ **Testing:** Mock từng context riêng lẻ
> - ❌ **Provider Hell:** Nhiều lớp lồng nhau
>   ```jsx
>   <ThemeProvider>
>     <LanguageProvider>
>       <AuthProvider>
...
> ```
>
> Hoặc dùng composition pattern / reduce.
>
> **Trade-off chấp nhận:** Multiple contexts cho app production. Single context chỉ dùng cho prototype hoặc app cực nhỏ (< 10 components).

> **💡 Mẹo phỏng vấn:** Khi trả lời Senior, hãy nhấn mạnh **"Context là transport, không phải state management"** và đưa ra **concrete scenario** (ví dụ: "Nếu app có real-time dashboard với 10 widget cập nhật 60fps, tôi sẽ không dùng Context mà dùng Zustand với selectors để tránh re-render không cần thiết").
