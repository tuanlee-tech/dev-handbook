...
// ============================================
// CONFIG - Có thể tùy chỉnh rule từ bên ngoài
// ============================================
...
const ALLOWED_EMAIL_DOMAINS = ['company.com', 'enterprise.vn', 'corp.net'];

const PHONE_COUNTRY_RULES = {
  US: { regex: /^\+1\d{10}$/, example: '+11234567890' },
  VN: { regex: /^\+84\d{9,10}$/, example: '+84912345678' },
  UK: { regex: /^\+44\d{10}$/, example: '+447123456789' },
};

// ============================================
// ZOD SCHEMAS VỚI VALIDATION NÂNG CAO
// ============================================

// Hàm tạo password schema động theo config
function createPasswordSchema(config: typeof PASSWORD_CONFIG) {
  let schema = z.string().min(config.minLength, `Mật khẩu phải có ít nhất ${config.minLength} ký tự`);

  if (config.requireUppercase) {
    schema = schema.regex(/[A-Z]/, 'Phải chứa chữ hoa');
  }
  if (config.requireLowercase) {
    schema = schema.regex(/[a-z]/, 'Phải chứa chữ thường');
  }
  if (config.requireNumber) {
    schema = schema.regex(/[0-9]/, 'Phải chứa số');
  }
  if (config.requireSpecial) {
    schema = schema.regex(/[^A-Za-z0-9]/, 'Phải chứa ký tự đặc biệt');
  }

  return schema;
}

// Email domain validation - chỉ chấp nhận domain công ty
const emailSchema = z
  .string()
  ...
  .refine(
    ...
    {
      message: `Email phải thuộc domain: ${ALLOWED_EMAIL_DOMAINS.join(', ')}`,
    }
  );

// Phone validation theo quốc gia
const phoneSchema = z.string().refine(
  (phone) => {
    return Object.values(PHONE_COUNTRY_RULES).some((rule) => rule.regex.test(phone));
  },
  {
    message: `Số điện thoại phải có mã quốc gia. VD: ${Object.values(PHONE_COUNTRY_RULES)
      .map((r) => r.example)
      .join(', ')}`,
  }
);
...
const accountSchema = z
  .object({
    username: z
      ...
      .regex(/^[a-zA-Z0-9_]+$/, 'Chỉ cho phép chữ, số và gạch dưới'),
    password: createPasswordSchema(PASSWORD_CONFIG),
    confirmPassword: z.string(),
  })
  .refine((data) => data.password === data.confirmPassword, {
...
const preferencesSchema = z.object({
  ...
  notifications: z.enum(['all', 'important', 'none']),
  language: z.enum(['en', 'vi', 'fr', 'es']),
  timezone: z.string().min(1, 'Múi giờ bắt buộc'),
});
...
type WizardAction =
  | { type: 'NEXT_STEP'; payload: { stepKey: keyof FormDataShape; data: any } }
  | { type: 'PREVIOUS_STEP' }
  | { type: 'GO_TO_STEP'; payload: number }
  | { type: 'SUBMIT_START' }
  | { type: 'SUBMIT_SUCCESS' }
  | { type: 'LOAD_DATA'; payload: Partial<WizardState> }
  | { type: 'RESET' };
...
const STORAGE_KEY = 'registration_wizard_hw1';
  ...
  switch (action.type) {
    case 'NEXT_STEP':
      return {
        ...
        formData: { ...state.formData, [action.payload.stepKey]: action.payload.data },
      };
    case 'PREVIOUS_STEP':
...
function ProgressBar({ currentStep }: { currentStep: number }) {
  const totalSteps = 5;
  const progress = ((currentStep + 1) / totalSteps) * 100;
  const labels = ['Cá nhân', 'Liên hệ', 'Tài khoản', 'Tùy chọn', 'Xác nhận'];
  ...
  return (
    <form onSubmit={handleSubmit(onNext)}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div>
          <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>Tên *</label>
          <input
            {...register('firstName')}
            style={{
              width: '100%',
          ...
          />
          {errors.firstName && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.firstName.message}</p>}
        </div>
        <div>
          <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>Họ *</label>
          <input
            {...register('lastName')}
            style={{
              width: '100%',
          ...
          />
          {errors.lastName && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.lastName.message}</p>}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        />
        {errors.birthDate && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.birthDate.message}</p>}
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        {errors.gender && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.gender.message}</p>}
      </div>

      <NavigationButtons onBack={() => {}} isFirstStep={true} isLastStep={false} isValid={isValid} />
    </form>
  );
}

// ============================================
// STEP 2: CONTACT VỚI EMAIL DOMAIN & PHONE COUNTRY
// ============================================
...
function Step2_Contact({
  ...
  return (
    <form onSubmit={handleSubmit(onNext)}>
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>
          Email công ty * <span style={{ fontWeight: 'normal', color: '#6b7280', fontSize: 12 }}>({ALLOWED_EMAIL_DOMAINS.join(', ')})</span>
        </label>
        <input
          type="email"
          {...register('email')}
        ...
        />
        {errors.email && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.email.message}</p>}
      </div>

      <div style={{ marginBottom: 16 }}>
        <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>
          Số điện thoại * <span style={{ fontWeight: 'normal', color: '#6b7280', fontSize: 12 }}>(có mã quốc gia)</span>
        </label>
        <input
          type="tel"
          {...register('phone')}
        ...
        />
        {errors.phone && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.phone.message}</p>}
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        />
        {errors.address && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.address.message}</p>}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div>
          <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>Thành phố *</label>
          <input
            {...register('city')}
            style={{
              width: '100%',
          ...
          />
          {errors.city && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.city.message}</p>}
        </div>
        <div>
          <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>Quốc gia *</label>
          <input
            {...register('country')}
            style={{
              width: '100%',
          ...
          />
          {errors.country && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.country.message}</p>}
        </div>
      </div>

      <NavigationButtons onBack={onBack} isFirstStep={false} isLastStep={false} isValid={isValid} />
    </form>
  );
}

// ============================================
// STEP 3: ACCOUNT VỚI ASYNC USERNAME & CUSTOM PASSWORD
// ============================================
...
function Step3_Account({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Giả lập API call
      const mockTakenUsernames = ['admin', 'root', 'user123', 'test'];
      setTimeout(() => {
        setUsernameAvailable(!mockTakenUsernames.includes(username.toLowerCase()));
  ...
  const getPasswordStrength = useCallback((pwd: string) => {
    if (!pwd) return { score: 0, label: 'Chưa nhập' };
    let score = 0;
    const checks = [
      pwd.length >= PASSWORD_CONFIG.minLength,
      PASSWORD_CONFIG.requireUppercase && /[A-Z]/.test(pwd),
      PASSWORD_CONFIG.requireLowercase && /[a-z]/.test(pwd),
      PASSWORD_CONFIG.requireNumber && /[0-9]/.test(pwd),
      PASSWORD_CONFIG.requireSpecial && /[^A-Za-z0-9]/.test(pwd),
    ];
    score = checks.filter(Boolean).length;

    const labels = ['Rất yếu', 'Yếu', 'Trung bình', 'Khá', 'Mạnh'];
    const colors = ['#dc2626', '#ef4444', '#d97706', '#3b82f6', '#059669'];
    return { score, label: labels[score - 1] || 'Rất yếu', color: colors[score - 1] || '#dc2626' };
  }, []);

  const strength = getPasswordStrength(password || '');

  const isUsernameValid = usernameAvailable === true && !errors.username;

  return (
    <form onSubmit={handleSubmit(onNext)}>
      <h2 style={{ marginBottom: 24 }}>Thông tin tài khoản</h2>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input
          ...
          style={{
            ...
            padding: 12,
            border: `1px solid ${
              errors.username ? '#dc2626' : usernameAvailable === false ? '#dc2626' : usernameAvailable === true ? '#059669' : '#d1d5db'
            }`,
            borderRadius: 8,
            fontSize: 16,
          }}
        />
        <div style={{ marginTop: 4, fontSize: 13, minHeight: 18 }}>
          {usernameChecking && <span style={{ color: '#6b7280' }}>⏳ Đang kiểm tra...</span>}
          {!usernameChecking && usernameAvailable === true && <span style={{ color: '#059669' }}>✅ Username khả dụng</span>}
          {!usernameChecking && usernameAvailable === false && <span style={{ color: '#dc2626' }}>❌ Username đã được sử dụng</span>}
          {errors.username && <span style={{ color: '#dc2626' }}>{errors.username.message}</span>}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>
          ...
          <span style={{ fontSize: 12, color: '#6b7280' }}>
            (Tối thiểu {PASSWORD_CONFIG.minLength} ký tự
            {PASSWORD_CONFIG.requireUppercase ? ', có hoa' : ''}
            {PASSWORD_CONFIG.requireLowercase ? ', thường' : ''}
            {PASSWORD_CONFIG.requireNumber ? ', số' : ''}
            {PASSWORD_CONFIG.requireSpecial ? ', đặc biệt' : ''})
          </span>
        </label>
        <input
          type="password"
        ...
        {password && (
          <div style={{ marginTop: 8 }}>
            ...
            <p style={{ fontSize: 12, color: strength.color }}>Độ mạnh: {strength.label}</p>
          </div>
        )}

        {errors.password && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.password.message}</p>}
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        />
        {errors.confirmPassword && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.confirmPassword.message}</p>}
      </div>
...
function Step4_Preferences({
  ...
  return (
    <form onSubmit={handleSubmit(onNext)}>
      <h2 style={{ marginBottom: 24 }}>Tùy chọn</h2>

      <div style={{ marginBottom: 24 }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', padding: 16, backgroundColor: '#f9fafb', borderRadius: 8 }}>
          <input type="checkbox" {...register('newsletter')} style={{ width: 20, height: 20 }} />
          <div>
            <div style={{ fontWeight: 500 }}>Đăng ký nhận bản tin</div>
            <div style={{ fontSize: 14, color: '#6b7280' }}>Nhận thông báo về tính năng mới và khuyến mãi</div>
          </div>
        </label>
      </div>

      <div style={{ marginBottom: 16 }}>
        <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>Thông báo *</label>
        <select {...register('notifications')} style={{ width: '100%', padding: 12, borderRadius: 8, fontSize: 16, border: `1px solid ${errors.notifications ? '#dc2626' : '#d1d5db'}` }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>Ngôn ngữ *</label>
        <select {...register('language')} style={{ width: '100%', padding: 12, borderRadius: 8, fontSize: 16, border: `1px solid ${errors.language ? '#dc2626' : '#d1d5db'}` }}>
          <option value="en">English</option>
          <option value="vi">Tiếng Việt</option>
          <option value="fr">Français</option>
          <option value="es">Español</option>
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: 'block', marginBottom: 8, fontWeight: 500 }}>Múi giờ *</label>
        <select {...register('timezone')} style={{ width: '100%', padding: 12, borderRadius: 8, fontSize: 16, border: `1px solid ${errors.timezone ? '#dc2626' : '#d1d5db'}` }}>
          <option value="Asia/Ho_Chi_Minh">Vietnam (ICT)</option>
          <option value="America/New_York">Eastern Time (ET)</option>
          <option value="America/Los_Angeles">Pacific Time (PT)</option>
          <option value="Europe/London">London (GMT)</option>
        ...
        {errors.timezone && <p style={{ color: '#dc2626', fontSize: 14, marginTop: 4 }}>{errors.timezone.message}</p>}
      </div>
...
function Step5_Review({
  ...
  const sections = [
    { key: 'personalInfo', label: 'Thông tin cá nhân', step: 0, data: formData.personalInfo },
    { key: 'contact', label: 'Liên hệ', step: 1, data: formData.contact },
    { key: 'account', label: 'Tài khoản', step: 2, data: formData.account },
    { key: 'preferences', label: 'Tùy chọn', step: 3, data: formData.preferences },
  ];

  return (
    <div>
      <h2 style={{ marginBottom: 24 }}>Xác nhận thông tin</h2>
      <p style={{ color: '#6b7280', marginBottom: 32 }}>Vui lòng kiểm tra lại thông tin trước khi gửi. Nhấn "Sửa" để thay đổi.</p>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <button
              ...
              style={{ padding: '6px 12px', backgroundColor: 'white', border: '1px solid #d1d5db', borderRadius: 6, cursor: 'pointer', fontSize: 14 }}
            >
              Sửa
            </button>
          </div>
          <div style={{ fontSize: 14, color: '#374151' }}>
            {section.data && Object.entries(section.data).map(([k, v]) => (
              <div key={k} style={{ marginBottom: 4 }}>
                ...
                <span style={{ fontWeight: 500 }}>{typeof v === 'boolean' ? (v ? 'Có' : 'Không') : String(v)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding: 24, backgroundColor: '#eff6ff', border: '1px solid #3b82f6', borderRadius: 8, marginBottom: 24 }}>
        <label style={{ display: 'flex', alignItems: 'start', gap: 12, cursor: 'pointer' }}>
          <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} style={{ marginTop: 4, width: 20, height: 20 }} />
          <span style={{ fontSize: 14, color: '#1e40af' }}>
            Tôi đồng ý với Điều khoản Dịch vụ và Chính sách Bảo mật. Tôi hiểu rằng thông tin của tôi sẽ được xử lý theo các điều khoản này.
          </span>
        </label>
      </div>

      <NavigationButtons onBack={onBack} isFirstStep={false} isLastStep={true} isValid={agreed} />
    </div>
  );
}
...
function SuccessScreen({ formData, onReset }: { formData: FormDataShape; onReset: () => void }) {
  return (
    <div style={{ textAlign: 'center', padding: 60, backgroundColor: '#f0fdf4', borderRadius: 12, border: '2px solid #86efac' }}>
      <div style={{ fontSize: 64, marginBottom: 24 }}>✅</div>
      <h2 style={{ color: '#059669', marginBottom: 16 }}>Đăng ký thành công!</h2>
      <p style={{ color: '#166534', fontSize: 18, marginBottom: 24 }}>
        Chào mừng, {formData.personalInfo?.firstName}!
      </p>
      <p style={{ color: '#6b7280', fontSize: 14, marginBottom: 24 }}>
        Email xác nhận đã được gửi đến {formData.contact?.email}
      </p>
      <button
        onClick={onReset}
        style={{ padding: '12px 24px', backgroundColor: '#3b82f6', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 16 }}
      >
        Đăng ký mới
      </button>
    </div>
...
export default function RegistrationWizard() {
  ...
  const { currentStep, formData, isSubmitting, isSubmitted } = state;
  ...
  const handleNext = (stepKey: keyof FormDataShape, data: any) => {
    dispatch({ type: 'NEXT_STEP', payload: { stepKey, data } });
  };
  ...
  if (isSubmitted) {
    return (
      <div style={{ maxWidth: 800, margin: '40px auto', padding: 40 }}>
  ...
  return (
    <div style={{ maxWidth: 800, margin: '40px auto', padding: 40, backgroundColor: 'white', borderRadius: 12, boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
      <h1 style={{ marginBottom: 32, textAlign: 'center' }}>Tạo tài khoản</h1>
      ...
      {currentStep === 1 && (
        <Step2_Contact
          defaultValues={formData.contact}
          onNext={(data) => handleNext('contact', data)}
          onBack={handleBack}
        />
      )}
      ...
      {currentStep === 2 && (
        <Step3_Account
          defaultValues={formData.account}
          onNext={(data) => handleNext('account', data)}
          onBack={handleBack}
        />
      )}
      ...
      {isSubmitting && (
        ...
        >
          <div style={{ padding: 40, backgroundColor: 'white', borderRadius: 12, textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>⏳</div>
            <div style={{ fontSize: 18, fontWeight: 500 }}>Đang gửi đăng ký...</div>
          </div>
        </div>
      )}
    </div>
