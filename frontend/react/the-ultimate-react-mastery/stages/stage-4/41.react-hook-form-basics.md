...
const RadioQuestion = ({ question, register, error }) => (
...
const RatingQuestion = ({ question, register, error, setValue, watch }) => {
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        {Array.from(
          ...
          <button
            ...
            type='button'
            onClick={() =>
              setValue(question.id, num.toString(), {
                shouldValidate: true,
                shouldDirty: true,
              })
            }
            aria-pressed={value === num.toString()}
            style={{
              width: '2.5rem',
              height: '2.5rem',
              borderRadius: '0.5rem',
              border: '2px solid',
              borderColor: value === num.toString() ? '#3b82f6' : '#d1d5db',
              backgroundColor: value === num.toString() ? '#eff6ff' : '#ffffff',
              color: value === num.toString() ? '#3b82f6' : '#374151',
              fontWeight: 600,
              cursor: 'pointer',
              fontSize: '1rem',
            }}
            aria-label={`Rate ${num} out of ${question.max}`}
          >
            {num}
          </button>
        ))}
        ...
        {...register(question.id, {
          ...
          validate: (value) => {
            ...
            if (isNaN(num) || num < question.min || num > question.max) {
              return `Rating must be between ${question.min} and ${question.max}`;
            }
            return true;
          },
        })}
...
const CheckboxQuestion = ({ question, register, error }) => (
  <fieldset style={{ border: 'none', padding: 0, margin: 0 }}>
    <legend
      style={{
        fontWeight: 600,
...
const TextareaQuestion = ({ question, register, error }) => (
  <div>
    <label
      htmlFor={question.id}
      style={{
...
export default function SurveyForm() {
  ...
  // Lấy form values qua useWatch với control
  const formValues = useWatch({ control });

  // Calculate visible questions based on conditions
  const visibleQuestions = useMemo(() => {
    return SURVEY_CONFIG.questions.filter((question) => {
      if (!question.condition) return true;
      return question.condition(formValues || {});
    });
  }, [formValues]);
  ...
  const progress = useMemo(() => {
    ...
    return {
      current: Math.min(answered + 1, totalVisible),
      total: totalVisible,
      percentage: totalVisible > 0 ? (answered / totalVisible) * 100 : 0,
    };
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        // defer UI update (tránh sync setState)
        setTimeout(() => {
          setSaveStatus('Draft restored from previous session');
        }, 0);
      ...
      setTimeout(() => {
        setLoadError('Could not load previous draft. Starting fresh.');
      }, 0);
      setTimeout(() => setLoadError(null), 5000);
    }
  }, [reset]);

  // Auto-save - trigger bởi formValues thay đổi
  useEffect(() => {
    // Chỉ auto-save sau khi đã có interaction (isDirty)
    if (!isDirty) return;
    ...
    autoSaveTimeoutRef.current = setTimeout(() => {
      try {
        ...
        localStorage.setItem(SURVEY_STORAGE_KEY, JSON.stringify(currentValues));
        ...
        >
          ...
          >
            {Object.entries(submittedData).map(([key, value]) => {
              const question = SURVEY_CONFIG.questions.find(
                (q) => q.id === key,
              );
              return (
                <div
  ...
  return (
    ...
    >
      ...
      <form
        onSubmit={handleSubmit(onSubmit)}
        noValidate
        aria-label='Customer satisfaction survey form'
      >
        <div
          role='group'
      ...
      >
        <p>
          Your responses are automatically saved. You can return later to
          complete the survey.
        </p>
      </footer>
    </div>
...
function BuggyForm() {
  ...
  // BUG: watch() return value không stable → gây dependency change liên tục
...
**Fix 4: useWatch + debounce (best practice)**

Khác với watch():

- `useWatch` subscribe vào form state
- chỉ re-render khi giá trị thực sự thay đổi
- không tạo object mới vô nghĩa mỗi render

Nên:

> values stable hơn
> → dependency không bị trigger liên tục
> → không loop

```jsx
// ✅ Cách 4: Dùng useWatch thay vì watch
 ...
 *   control,
 *   name: ['name', 'email'], // chỉ watch field cần thiết
 * });
 */
...
export default function CurrencyCalculator() {
  ...
  // ✅ Derive validation without calling watch()
  const currenciesAreDifferent = fromCurrency !== toCurrency;
  ...
  useEffect(() => {
    if (fromCurrency && toCurrency && fromCurrency !== toCurrency) {
      ...
      if (rate) {
        setValue('exchangeRate', rate.toFixed(5));
      }
    }
  ...
  const swapCurrencies = () => {
    ...
    setValue('fromCurrency', values.toCurrency);
    setValue('toCurrency', values.fromCurrency);
    setResult(null);
  };
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
      ...
      >
        ...
        <button
          type='submit'
          disabled={!isValid || isConverting || !currenciesAreDifferent}
          style={{
            width: '100%',
            padding: '16px',
...
useEffect(() => {
  const subscription = watch((values, { name, type }) => {
    ...
    // Ví dụ: auto-format hoặc side effect
    if (name === 'email') {
      const cleaned = values.email?.trim();
...
// Watch toàn form (chỉ dùng khi thật cần)
...
- `watch()` trong render → dễ gây re-render không cần thiết
- `useEffect + watch()` → verbose, dễ sai dependency
- `useWatch()` → sạch, reactive, đúng chuẩn React Hook Form
...
1. **Q:** Sự khác biệt giữa controlled và uncontrolled components?
   ...
   **A:**
   Controlled: React state control value (useState).

   Uncontrolled: DOM controls value (refs).

   RHF uses uncontrolled approach for better performance.
...
3. **Q:** Tại sao React Hook Form performance tốt hơn controlled forms?
...
4. **Q:** Làm sao handle conditional validation?
   ...
   **A:** Dùng `validate` function với access đến other field values qua `watch()`, `useWatch()`, `getValues()`.

**Senior Level:**

5. **Q:** Design form system cho large application với nhiều loại forms khác nhau?

   **A:**
   - Quy tắc validation dùng chung\*\* (custom hooks)
   - Component field tái sử dụng
   - Cách tiếp cận dựa trên cấu hình form (form config-driven)
   - Tích hợp xử lý lỗi từ API
   - Hiển thị lỗi tập trung (centralized error display)
   - Theo dõi phân tích hành vi người dùng (analytics tracking)

6. **Q:** Trade-offs giữa React Hook Form vs Formik?

   **A:**
   - RHF: Better performance (dùng uncontrolled inputs), bundle size nhỏ hơn, modern API
   - Formik: More mature (trưởng thành), hệ sinh thái lớn, được nhiều lập trình viên quen dùng
   - Việc lựa chọn phụ thuộc vào: team familiarity, bundle size constraints, existing codebase
...
```
Tình huống: Form đăng ký với 15 field, mỗi field dùng useState.
Vấn đề: Gõ vào email → toàn bộ form re-render → bị lag!

Giải pháp: Chuyển sang React Hook Form (RHF) → cải thiện hiệu năng ~10x.

Bài học: Controlled forms không phù hợp để scale cho các form lớn.
```

**Story 2: The Lost Data**

```
Tình huống: Người dùng điền form trong 10 phút, trình duyệt bị crash.

Vấn đề: Toàn bộ dữ liệu bị mất vì không có cơ chế tự động lưu.

Giải pháp: Tự động lưu (auto-save) vào localStorage mỗi 5 giây bằng cách dùng useWatch(), getValues(), watch() trong React Hook Form.

Bài học: Luôn nên triển khai lưu bản nháp (draft save) cho các form dài hoặc phức tạp.
```

**Story 3: The Validation Hell**

```
Tình huống: Xác thực mật khẩu với 8 quy tắc khác nhau.

Vấn đề: Có 8 hàm validation riêng → xử lý lỗi phức tạp, code khó quản lý.

Giải pháp: Sử dụng validate object trong React Hook Form với các rule được đặt tên rõ ràng.

Bài học: validate object của RHF giúp code gọn gàng và dễ bảo trì hơn rất nhiều.
```
