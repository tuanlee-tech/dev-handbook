...
describe('Authentication Integration Test', () => {
  test('user can login and profile updates from real AuthProvider state', async () => {
    server.use(
    ...
    expect(screen.getByText(/email: john@example.com/i)).toBeInTheDocument();
    expect(
      screen.queryByText(/please login to continue/i),
    ).not.toBeInTheDocument();
  });

  test('failed login shows error and keeps user logged out', async () => {
    server.use(
      rest.post('/api/login', async (req, res, ctx) => {
        return res(
          ctx.status(401),
  ...
  test('user can logout after successful login', async () => {
    server.use(
      rest.post('/api/login', async (req, res, ctx) => {
        return res(
          ctx.status(200),
  ...
  test('loading state disappears after login request finishes', async () => {
    server.use(
      rest.post('/api/login', async (req, res, ctx) => {
        return res(
          ctx.delay(300),
 ...
 *
 * 1. Unit Tests
 * - Dùng cho component nhỏ, logic rõ ràng, ít phụ thuộc component khác.
 * - Mục tiêu: test rendering, props, disabled state, callback.
 *
 * Components nên unit test:
 * - ProductCard
 * - CartItem
 * - CheckoutForm
 * - OrderConfirmation
 *
 * 2. Integration Tests
 * - Dùng cho flow có nhiều component dùng chung state.
 * - Mục tiêu: verify data flow giữa ProductList -> Cart -> Checkout -> OrderConfirmation.
 *
 * Main flows nên integration test:
...
const ProductCard = ({ product, onAddToCart }) => {
  ...
  return (
    <article aria-label={product.name}>
      <h3>{product.name}</h3>
      <p>${product.price}</p>
      <p>{isOutOfStock ? 'Out of stock' : `${product.stock} in stock`}</p>
...
const CheckoutForm = ({ disabled = false, onSubmit }) => {
  const [formData, setFormData] = useState({
    name: '',
    email: '',
    address: '',
...
const CheckoutSection = () => {
  const { items, checkout, checkoutError, checkingOut } = useCart();

  if (items.length === 0) {
    return <p>Add products to cart before checkout</p>;
  }
  ...
  return (
    <section>
      ...
      {checkingOut && <p>Processing order...</p>}

      {checkoutError && <div role='alert'>{checkoutError}</div>}
    </section>
  );
};
...
const ECommerceApp = ({ products = mockProducts }) => {
  return (
    <CartProvider>
...
describe('ProductCard - Unit Tests', () => {
  ...
  test('calls onAddToCart when clicking add button', () => {
  ...
  test('disables add button when product is out of stock', () => {
    render(
      <ProductCard
...
describe('CartItem - Unit Tests', () => {
  ...
  test('renders cart item information', () => {
    render(
      <CartItem
        item={cartItem}
        onIncrease={jest.fn()}
  ...
  test('calls quantity and remove handlers', () => {
...
describe('CheckoutForm - Unit Tests', () => {
  ...
  test('submits customer data when form is valid', () => {
...
describe('OrderConfirmation - Unit Tests', () => {
  test('renders nothing when order does not exist', () => {
    const { container } = render(<OrderConfirmation order={null} />);
  ...
  test('renders order confirmation details', () => {
    ...
    expect(screen.getByText(/order id: order-123/i)).toBeInTheDocument();
    expect(screen.getByText(/total paid: \$1250/i)).toBeInTheDocument();
  });
});

describe('E-commerce Flow - Integration Tests', () => {
  test('user can browse products and add product to cart', () => {
    render(<ECommerceApp />);
    ...
    const laptop = screen.getByRole('article', { name: /laptop pro/i });

    fireEvent.click(
      within(laptop).getByRole('button', { name: /add to cart/i }),
    );

    const cart = screen.getByRole('region', { name: /cart/i });

    expect(within(cart).getByText(/laptop pro/i)).toBeInTheDocument();
    expect(within(cart).getByText(/quantity: 1/i)).toBeInTheDocument();
    expect(within(cart).getByText(/total: \$1200/i)).toBeInTheDocument();
  });

  test('adding same product twice increases quantity and total', () => {
    render(<ECommerceApp />);
    ...
    expect(within(cart).getByText(/quantity: 2/i)).toBeInTheDocument();
    expect(within(cart).getByText(/total: \$2400/i)).toBeInTheDocument();
  });

  test('user can increase, decrease, and remove cart items', () => {
    render(<ECommerceApp />);

    const mouse = screen.getByRole('article', { name: /wireless mouse/i });

    fireEvent.click(
      within(mouse).getByRole('button', { name: /add to cart/i }),
    );

    const cart = screen.getByRole('region', { name: /cart/i });

    expect(within(cart).getByText(/quantity: 1/i)).toBeInTheDocument();
    expect(within(cart).getByText(/total: \$50/i)).toBeInTheDocument();

    fireEvent.click(within(cart).getByRole('button', { name: '+' }));

    expect(within(cart).getByText(/quantity: 2/i)).toBeInTheDocument();
    expect(within(cart).getByText(/total: \$100/i)).toBeInTheDocument();
    ...
    fireEvent.click(within(cart).getByRole('button', { name: /remove/i }));

    expect(screen.getByText(/your cart is empty/i)).toBeInTheDocument();
    expect(screen.queryByText(/total: \$50/i)).not.toBeInTheDocument();
  });

  test('out-of-stock product cannot be added to cart', () => {
    render(<ECommerceApp />);

    const keyboard = screen.getByRole('article', {
      name: /mechanical keyboard/i,
    });

    expect(
      within(keyboard).getByRole('button', { name: /add to cart/i }),
    ).toBeDisabled();
    ...
    expect(
      within(cart).queryByText(/mechanical keyboard/i),
    ).not.toBeInTheDocument();
    expect(screen.getByText(/your cart is empty/i)).toBeInTheDocument();
  });

  test('checkout is blocked when cart is empty', () => {
    render(<ECommerceApp />);

    expect(
      screen.getByText(/add products to cart before checkout/i),
    ).toBeInTheDocument();
    expect(
  ...
  test('successful checkout creates order confirmation and clears cart', async () => {
    server.use(
    ...
    fireEvent.click(
      within(screen.getByRole('article', { name: /laptop pro/i })).getByRole(
        'button',
        {
          name: /add to cart/i,
        },
    ...
    fireEvent.click(
      within(
        screen.getByRole('article', { name: /wireless mouse/i }),
      ).getByRole('button', {
        name: /add to cart/i,
      }),
    );

    expect(screen.getByText(/total: \$1250/i)).toBeInTheDocument();
  ...
  test('failed checkout shows error and keeps cart items', async () => {
    server.use(
      rest.post('/api/checkout', async (req, res, ctx) => {
        return res(
          ctx.status(500),
    ...
    expect(error).toHaveTextContent(/payment service unavailable/i);
  ...
  test('empty product list shows empty state', () => {
    render(<ECommerceApp products={[]} />);

    expect(screen.getByText(/no products found/i)).toBeInTheDocument();
    expect(screen.getByText(/your cart is empty/i)).toBeInTheDocument();
  });

  test('invalid checkout form shows validation error and does not call checkout API', async () => {
  ...
  test('loading state disables checkout button during checkout request', async () => {
    server.use(
      rest.post('/api/checkout', async (req, res, ctx) => {
        return res(
          ctx.delay(300),
    ...
    await waitFor(() => {
      expect(screen.queryByText(/processing order/i)).not.toBeInTheDocument();
    });
 ...
 *
 * Unit Tests:
 * - ProductCard: because product display and disabled state are isolated UI behavior.
 * - CartItem: because quantity/remove buttons only need callback verification.
 * - CheckoutForm: because validation can be tested without real cart state.
 * - OrderConfirmation: because it only renders order props.
 *
 * Integration Tests:
 * - Product browsing + add to cart: because ProductList and CartSummary must share Context state.
 * - Quantity updates: because cart total depends on state changes across components.
 * - Checkout success: because cart state, API call, order state, and confirmation UI must work together.
 * - Checkout error: because failed API should keep cart data and show recoverable error.
 * - Empty states: because user flow should remain clear when product/cart data is empty.
 *
 * Recommended production split:
 * - 70% Unit Tests
 * - 20% Integration Tests
 * - 10% E2E Tests for critical checkout journey
 */
```
