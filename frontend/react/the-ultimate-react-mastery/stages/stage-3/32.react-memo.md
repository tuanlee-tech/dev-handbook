...
**Kịch bản:**
Bạn có một dashboard với nhiều widgets. Khi user thay đổi date filter, chỉ DatePicker và Chart widget cần update. Nhưng tất cả 20 widgets khác cũng re-render!
...
- User thay đổi date → `dateRange` state thay đổi
- Dashboard re-render
...
YES → Compare props
    ...
    │
    └─ Props SAME → Skip render, reuse previous result ✅
```
...
Chef: *Makes fresh burger again* ← Lãng phí!
```
...
Chef: *Makes fresh burger with cheese* → Updates warmer
```
...
// DOM vẫn update nếu component thực sự render!
```
...
// Tất cả đều quá đơn giản, không cần memo!
```
...
// React.memo không giúp gì vì config là new object mỗi lần!
```
...
└─ Result: Both render (expected, props actually changed)
```
...
2. **React.memo still re-renders when props change**
   ...
   ```jsx
   // name prop: 'John' → 'Jane'
   // → MemoizedChild renders (correct behavior)
   ```
...
🟠 ArrayPropsChild rendered          ← Rendered! New array ❌
```
...
**✅ Solutions (preview, will learn tomorrow):**
...
(🟢 CustomMemoChild NOT logged)     ← Skipped! Custom logic works ✅
```
...
<details>
<summary>✅ Solution & Rationale</summary>
...
### ⭐⭐⭐ Bài 3: Product List Optimization (40 phút)
...
### ⭐⭐⭐⭐⭐ Bài 5: Production-Ready Memo Strategy (90 phút)
...
**Kết quả mong đợi khi chạy (console logs):**
...
// → StaticInfo, UserGreeting, QuickStats, RecentActivity, AlertSummary đều skip
```

**Ghi chú upgrade trong tương lai (không dùng bây giờ):**
...
→ Chiến lược memo hiện tại đã tối ưu khá tốt trong giới hạn kiến thức đến ngày 32.
...
<MemoChild name="John" age={30} /> {/* ✅ Primitives stable */}
```
...
// 🐛 App actually SLOWER with memo! Why?
```
...
1. Identify components nên memo
2. Apply memo
...
**Kết quả ví dụ khi chạy (console logs):**
...
// → Static parts (FilterButton "all" và "completed") skip render khi không liên quan
```

**Quyết định memo hóa:**

- `TodoList` → memo vì chứa danh sách, giúp giảm render khi filter thay đổi nhưng todos không đổi
- `TodoItem` → memo + custom compare vì:
  - Có thể hàng chục/hàng trăm item
  - onToggle/onDelete là function mới mỗi render → cần custom compare
- `FilterButton` → memo + custom compare vì:
  - Chỉ 3 nút, nhưng giúp skip render khi thêm/xóa todo (chỉ current thay đổi)
...
// → FilterButton không cần custom compare nữa
```
...
export default MemoPerformanceTester;
```

**Kết quả ví dụ khi chạy (thời gian thực tế phụ thuộc máy):**
...
Trạng thái: Custom memo nhanh nhất
```
...
1. React.memo có overhead nhỏ (so sánh props) → chỉ đáng dùng khi render thực sự nặng (> vài ms mỗi lần)
2. Khi props không đổi và component đắt đỏ → memo tiết kiệm rất nhiều
3. Khi props luôn thay đổi hoặc component quá đơn giản → memo làm chậm hơn
4. Custom compare có thể rất mạnh (nhưng dễ sai và khó bảo trì)
...
// → Khi đó React.memo default sẽ hoạt động tốt hơn, không cần custom compare
```
