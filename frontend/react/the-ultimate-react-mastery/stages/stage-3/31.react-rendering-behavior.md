...
function DependentChild({ value }) {
  return (
    ...
    >
      ...
      <p>Nhận props → vẫn render mỗi khi parent render (trừ khi dùng memo)</p>
    </div>
  );
}
  ...
  return (
    <div style={{ margin: '20px 0' }}>
      ...
      <p style={{ color: '#555', marginTop: 8, fontSize: '14px' }}>
        Mỗi ký tự gõ → chỉ component này re-render (state local)
      </p>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ul>
          <li>
            Mỗi lần state thay đổi →{' '}
            <strong>component đó và toàn bộ con cháu của nó render lại</strong>
          </li>
          <li>Component không nhận props vẫn render → lãng phí</li>
          <li>
            Component nhận props vẫn render khi parent render → lãng phí nếu
            props không đổi
          </li>
          <li>
            State local chỉ ảnh hưởng component hiện tại → không lan toàn cây
          </li>
        </ul>
        <p style={{ marginTop: 16, fontWeight: 'bold' }}>
          → React.memo có thể giảm phần lớn render không cần thiết
        </p>
      </div>
    </div>
  );
...
(Không có component nào khác render)

Nhấn +1 của A ở CounterPanel đầu tiên:
→ CounterPanel #M+1 (panel đầu)
→ StaticChild #M+1
→ DependentChild #M+1 (value = giá trị mới)
→ DependentChild #M+1 (value = 0)

→ CounterPanel thứ hai KHÔNG render

→ Rõ ràng thấy re-render chỉ xảy ra trong subtree chứa state
→ Nhưng vẫn có lãng phí do children luôn render theo parent
```
