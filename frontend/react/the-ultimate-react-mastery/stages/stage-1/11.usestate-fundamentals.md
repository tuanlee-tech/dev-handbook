...
function ToggleSwitchCorrect({ label, disabled = false }) {
...
// 🎯 NHIỆM VỤ CỦA BẠN:
function ToggleSwitch({ label, disabled = false }) {
  // TODO: Implement với useState

  return <div>{/* TODO: Implement toggle switch UI */}</div>;
}
```
