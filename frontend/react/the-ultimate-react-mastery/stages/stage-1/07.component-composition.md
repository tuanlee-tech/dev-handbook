...
<details>
<summary>💡 Solution (Simplified without Context)</summary>
...
| ----------------------- | ---------------------- | ----------------------------------------------------- | ------------------------------------------------- | ----------------------------------- |
| **Children Prop**       | Simple wrappers        | • Simple<br>• Flexible content                        | • No structure<br>• Hard to target specific parts | `<Card>{content}</Card>`            |
| **Named Slots**         | Structured layouts     | • Clear structure<br>• Easy to style parts            | • More props<br>• Verbose                         | `<Card header={...} body={...} />`  |
| **Compound Components** | Complex UIs            | • Flexible API<br>• Shared state<br>• Clear hierarchy | • More complex<br>• Learning curve                | `<Tabs><Tabs.Tab /></Tabs>`         |
| **Render Props**        | Logic reuse            | • Maximum flexibility<br>• Access to internals        | • Complex syntax<br>• Callback hell               | `<Mouse render={({x,y}) => ...} />` |
| **HOC**                 | Cross-cutting concerns | • Reuse logic<br>• Composition                        | • Props collision<br>• Wrapper hell               | `withAuth(Component)`               |
...
1. Composition - di chuyển component gần nơi dùng data
2. Context API - share data globally
3. State management libraries (Redux, Zustand)
4. Component specialization

**Senior Level:**

Q5: "Trade-offs giữa Compound Components và Render Props?"  
A:

- **Compound:** Clear API, easier to use, better DX. Nhưng cần shared state (Context).
- **Render Props:** Maximum flexibility, no setup. Nhưng complex syntax, harder to read.
- Modern: Prefer Custom Hooks cho logic reuse.
...
> "Junior dev tạo 'universal component' có thể làm MỌI THỨ: 50+ props, 10+ render props, config object khổng lồ. Không ai hiểu cách dùng! Code review: Break down thành smaller, specific components. Lesson: KISS > Clever abstractions!"

**Story 3: The Compound Components Win**

> "Redesign Table component: ban đầu dùng massive props object. Switching to compound pattern: `<Table><Table.Header /><Table.Row /></Table>`. Code readable tăng 10x, customization dễ hơn, bugs giảm. Team lead: 'This is the way!' Lesson: Right pattern = better DX!"
