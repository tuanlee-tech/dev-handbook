...
function SearchBar({ onSearch }) {
  const handleSubmit = (event) => {
    ...
    /*
     * Trong TS, e.target có type là `EventTarget` không có thuộc tính elements.
     * e.target.elements // ❌ TS sẽ báo lỗi
     * Nhưng với form submit thì thực tế e.currentTarget mới là <form>
     * Dùng e.currentTarget thay vì e.target
...
/**
 * Wrapper component that listens for productsFiltered event
 * Cập nhật grid bằng DOM manipulation (không dùng state/effect)
 */
function ProductGridWrapper({ initialProducts, onViewDetail }) {
  // Hàm render toàn bộ grid dựa trên mảng products nhận được
  const renderGrid = (productsToShow) => {
    const container = document.getElementById('product-grid-inner');
    if (!container) return;
    ...
    if (!productsToShow || productsToShow.length === 0) {
      container.innerHTML = `
        <div style="text-align: center; padding: 60px 20px; color: #64748b;">
          ...
          <h3 style="margin: 0 0 8px 0; font-size: 20px;">No products found</h3>
    ...
    productsToShow.forEach((product) => {
      ...
      html += `
        ...
        >
          <!-- Copy cấu trúc từ ProductCard của bạn -->
          <div style="position: relative;">
            <img src="${product.image}" alt="${product.name}" style="width:100%; height:200px; object-fit:cover;" />
            <span style="position:absolute; top:12px; left:12px; background:#1a1a2e; color:white; padding:4px 12px; border-radius:16px; font-size:12px; text-transform:capitalize;">
              ${product.category}
            </span>
            ${!product.inStock ? `<span style="position:absolute; top:12px; right:12px; background:#e63946; color:white; padding:4px 12px; border-radius:16px; font-size:12px;">Out of Stock</span>` : ''}
          </div>

          <div style="padding:16px;">
            <h3 style="margin:0 0 8px; font-size:18px; font-weight:600; color:#1a1a2e;">${product.name}</h3>
            <p style="margin:0 0 12px; font-size:14px; color:#64748b; line-height:1.5; height:42px; overflow:hidden; text-overflow:ellipsis; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;">
              ${product.description}
            </p>

            <div style="font-size:24px; font-weight:bold; color:#1a1a2e; margin-bottom:16px;">
              ${formattedPrice}
            </div>

            <div style="display:flex; gap:8px;">
              <button class="view-detail-btn" style="flex:1; padding:10px; background:#16213e; color:white; border:none; border-radius:8px; cursor:pointer;">
                View Details
              </button>
              <button 
                class="add-to-cart-btn" 
                ${product.inStock ? '' : 'disabled'}
                style="flex:1; padding:10px; background:${product.inStock ? '#0f62fe' : '#cbd5e1'}; color:white; border:none; border-radius:8px; cursor:${product.inStock ? 'pointer' : 'not-allowed'};"
              >
  ...
  // Render ban đầu
  setTimeout(() => renderGrid(initialProducts), 0);

  // Lắng nghe event (không dùng useEffect)
  const handleProductsFiltered = (event) => {
    const newProducts = event.detail;
    renderGrid(newProducts);
  };

  window.addEventListener('productsFiltered', handleProductsFiltered);

  // Vì đây là functional component, cleanup sẽ không tự động
  // Nhưng trong project này component không unmount thường xuyên → tạm chấp nhận
  // Nếu muốn cleanup chính xác hơn, có thể thêm logic khi component bị xóa (nhưng không cần thiết lúc này)
 ...
 * @param {Function} onSortChange - Callback khi thay đổi sort option
 *                             Nhận object { type: 'sort', value: string }
 */
...
```jsx
// Trong App.jsx - cập nhật phần currentFilters và các hàm liên quan

// Thêm vào currentFilters
let currentFilters = {
  category: 'all',
  priceRange: 'all',
  searchQuery: '',
...
// Cập nhật getFilteredProducts
const getFilteredProducts = () => {
  let filtered = [...products];

  // ... (giữ nguyên category, price, search)
...
// Trong JSX - thêm SortDropdown vào khu vực filter (cùng với FilterPanel hoặc bên dưới SearchBar)
<div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '32px' }}>
  <aside>
    <FilterPanel onFilterChange={handleFilterChange} />
    <SortDropdown onSortChange={handleSortChange} /> {/* ← thêm */}
  </aside>
  {/* ... */}
</div>;
```
...
```jsx
// Ví dụ kết quả sau khi chọn các sort khác nhau:

// Price: Low to High
// → Logitech MX Master 3S ($99) → AirPods Pro ($249) → ...

// Name: A to Z
// → AirPods Pro → Dell XPS 15 → Google Pixel 8 Pro → ...

// Rating: High to Low
// → iPhone 15 Pro (4.9) → Sony WH-1000XM5 (4.9) → MacBook Pro (4.8) → ...

// Lưu ý ngắn gọn:
// Cách này vẫn dùng closure + DOM hack như project gốc. Khi học `useState` (ngày 11), bạn chỉ cần thay `currentFilters` bằng state và bỏ hết phần `updateProductDisplay` + custom event.
```
...
/**
 * Hiển thị tối đa 4 sản phẩm đã xem gần đây
 * Không dùng useState, dùng localStorage + DOM manipulation + custom event
 *
 * @param {Function} onViewDetail - Callback khi click vào sản phẩm để xem chi tiết
 */
function RecentlyViewed({ onViewDetail }) {
  // Hàm lấy danh sách IDs từ localStorage
  const getRecentIds = () => {
    try {
  ...
  // Hàm cập nhật UI dựa trên IDs hiện tại
  const renderRecentProducts = () => {
    ...
    // Thêm event listener cho từng card
  ...
  // Render lần đầu khi mount
  setTimeout(renderRecentProducts, 0);

  // Lắng nghe event khi xem sản phẩm mới
  ...
  // Cleanup (dù trong React functional component không có cleanup trực tiếp ở đây)
  // Vì đây là component tĩnh, ta giả định không unmount thường xuyên
...
```jsx
// Trong App.jsx - giữ nguyên dispatch event như bài gốc

const handleViewDetail = (product) => {
  selectedProduct = product;

  // Dispatch event để RecentlyViewed cập nhật
...
// Trong JSX - đặt component ở vị trí mong muốn
...
```jsx
// Ví dụ kết quả:
// Sau khi xem iPhone 15 Pro → MacBook Pro → AirPods Pro → Dell XPS

// localStorage.getItem('recentlyViewed')
// → "[7,1,3,2]"

// Hiển thị theo thứ tự mới nhất → cũ nhất:
// AirPods Pro → MacBook Pro → iPhone 15 Pro → Dell XPS
```
...
/**
 * Thanh so sánh cố định ở dưới cùng màn hình (fixed bottom bar)
 * Hiển thị tối đa 3 sản phẩm đang so sánh
 * Không dùng useState, quản lý qua localStorage + DOM update
 *
 * @param {Function} onOpenCompare - Callback khi nhấn "Compare Now" để mở modal
 */
function ComparisonBar({ onOpenCompare }) {
  ...
  // Hàm render lại toàn bộ thanh bar
  ...
  // Lắng nghe event add to compare
  const handleAddToCompare = (event) => {
    const product = event.detail;
    if (!product?.id) return;

    let ids = getComparedIds();
    if (ids.includes(product.id)) return; // đã có thì không thêm

    ids = [...ids, product.id].slice(-3); // giữ tối đa 3, mới nhất ở cuối
...
/**
 * Modal so sánh chi tiết các sản phẩm
 * Không dùng state, render HTML động dựa trên localStorage
 *
    ...
    let html = `
      <div style="padding:24px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
          ...
          <button id="close-compare-modal" style="background:none; border:none; font-size:28px; cursor:pointer;">×</button>
        </div>
...
const handleOpenCompare = () => {
  ...
  // Gọi render lại nội dung modal
  const modal = document.querySelector('#comparison-modal-content');
  if (modal) {
    // Trigger render bằng cách gọi lại logic trong ComparisonModal nếu cần
    // Ở đây ta dựa vào setTimeout ban đầu + localStorage
  }
};
...
```jsx
// Trong ProductCard.jsx - nút Add to Compare (giữ nguyên như trước)
...
```jsx
// Ví dụ kết quả:
// - Add 3 sản phẩm → thanh bottom hiện 3 ảnh nhỏ + nút Compare Now active
// - Nhấn Compare Now → modal mở với bảng so sánh, các spec khác nhau highlight
// - Xóa 1 sản phẩm → bar tự update, nếu còn <2 thì nút Compare Now disable
```
...
```jsx
// src/components/WishlistBar.jsx  (icon + count ở Header)

/**
 * Icon Wishlist ở Header với số lượng sản phẩm
 * Click để mở modal wishlist
 */
function WishlistBar({ onOpenWishlist }) {
  ...
  // Lắng nghe thay đổi wishlist để cập nhật count
...
/**
 * Modal hiển thị danh sách sản phẩm trong wishlist
  ...
  const renderWishlistContent = () => {
    ...
    let html = `
      <div style="padding: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
          ...
          <button id="close-wishlist-modal" style="background: none; border: none; font-size: 28px; cursor: pointer;">×</button>
        </div>
    ...
    container.querySelectorAll('[data-action="remove"]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        // Phát event để cập nhật icon count
...
```jsx
// Trong ProductCard.jsx - thêm nút "Add to Wishlist"

<div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
  {/* Các nút cũ: View Details, Add to Cart, Compare */}
...
```jsx
// Trong App.jsx - tích hợp

// Thêm state-like variable
...
// Trong Header.jsx - thay phần cart icon bằng:
<div style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>
  {/* ... các link khác ... */}

  <WishlistBar onOpenWishlist={handleOpenWishlist} />

  {/* Cart icon giữ nguyên */}
  <div style={{ position: 'relative', cursor: 'pointer' }}>
    <span style={{ fontSize: '24px' }}>🛍️</span>
    {cartCount > 0 && ( /* ... */ )}
  </div>
...
// Thêm modal vào return của App (cuối cùng, cùng khu vực với các modal khác)
<WishlistModal onClose={handleCloseWishlist} />
```

```jsx
// Ví dụ kết quả:
// - Nhấn "Add to Wishlist" trên MacBook → count ở header tăng lên 1
// - Nhấn icon ❤️ → modal mở, hiển thị MacBook với nút View Details & Remove
// - Remove → count giảm về 0, modal hiển thị empty state
// - Dữ liệu vẫn giữ sau khi refresh trang (nhờ localStorage)
```
...
<details>
<summary>💡 Solution – Full E-commerce Features (Shopping Cart + Checkout + Order History)</summary>

### 1. Cập nhật ProductCard – Thêm nút Add to Cart (với quantity = 1)

```jsx
// Trong ProductCard.jsx – phần actions

<div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
  {/* View Details, Compare, Wishlist giữ nguyên */}
...
### 2. Helper functions cho Cart (dùng trong nhiều nơi)

```jsx
// src/utils/cart.js (tạo file mới)
...
```jsx
// Trong Header.jsx – phần cart icon
...
// Thêm logic render count (có thể đặt trong App.jsx hoặc Header)
...
### 4. Cart Section (hiển thị dưới dạng section trong main)
...
### 5. Multi-step Checkout (dùng hash để điều hướng)
...
### 7. Tích hợp vào App.jsx

```jsx
// Trong return của App

<main style={{ maxWidth: '1400px', margin: '0 auto', padding: '40px 20px' }}>
  {/* SearchBar, FilterPanel, ProductGrid giữ nguyên */}

  <CartSection />
  <CheckoutSection />
  <OrderHistory />

  {/* Các modal khác: ProductDetail, WishlistModal, ComparisonModal */}
</main>
```
...
- **Ngày 11**: useState → Refactor filters properly
- **Ngày 12**: useReducer → Complex state management
- **Ngày 17**: useEffect → Data fetching
- **Ngày 23**: Memoization → Performance optimization
...
**1. Quản lý State là yếu tố then chốt**

```
Không có state (dự án hiện tại):
- Các thủ thuật thao tác DOM
- Khó debug
- Không thể mở rộng

Có state (Ngày 11+):
- Cập nhật theo kiểu khai báo
- Dễ hiểu và dễ suy luận
- Có thể mở rộng
```

**2. Thiết kế Component rất quan trọng**

```
Component tốt:
- Một trách nhiệm duy nhất
- Có thể tái sử dụng
- Props được đặt tên rõ ràng
- Tự chứa
...
- Phụ thuộc chặt chẽ
- Props không rõ ràng
- Phụ thuộc vào state bên ngoài
```

**3. Trải nghiệm người dùng là trên hết**

```
Sự hoàn hảo về kỹ thuật < Sản phẩm hoạt động được
Giao diện đẹp < Giao diện dùng được
Nhiều tính năng < Các tính năng cốt lõi được làm tốt
```
...
**Q1**: "Hãy trình bày kiến trúc component của bạn"
**A**: Giải thích cấu trúc phân cấp, luồng dữ liệu, lý do tách các component nhất định

**Q2**: "Bạn sẽ thêm một tính năng mới như thế nào?"
**A**: Xác định các component bị ảnh hưởng, lập kế hoạch luồng dữ liệu, triển khai từng bước, kiểm thử

**Q3**: "Bạn sẽ refactor phần nào trước tiên?"
**A**: Logic lọc/tìm kiếm để sử dụng quản lý state phù hợp

---

## 🎯 PREVIEW NGÀY MAI

**Ngày 11: useState - Mảnh Ghép Còn Thiếu**

Ngày mai chúng ta sẽ giải quyết TẤT CẢ các giải pháp mang tính “chữa cháy” của hôm nay:

- ✅ Quản lý state đúng cách
- ✅ Không thao tác DOM
- ✅ Cập nhật theo kiểu khai báo
- ✅ React theo đúng “cách của React”

Chúng ta sẽ refactor dự án này bằng `useState` và xem nó trở nên gọn gàng hơn nhiều như thế nào!

**Chuẩn bị:**

- Xem lại code của hôm nay
- Xác định tất cả các “hack” đã sử dụng
- Nghĩ về những state mà chúng ta cần
- Hào hứng lên — đây là lúc React thực sự tỏa sáng! 🚀

---

**🎉 Chúc mừng! Bạn đã hoàn thành Week 2!**

Bạn vừa build một full project chỉ với kiến thức cơ bản. Từ ngày mai, mọi thứ sẽ trở nên powerful hơn nhiều với State Management! 💪

**Next Week Preview:**

- Ngày 11-15: State Management Mastery
- Ngày 16: Project 2 (với proper state!)

Rest well - Week 3 is going to be intense! 🔥
