...
/**
 * Load initial expenses từ localStorage (lazy)
 * @returns {Array} mảng expenses hoặc rỗng
 */
function loadExpenses() {
  try {
    ...
    // Đảm bảo date là string ISO hợp lệ
...
function ExpenseTracker() {
  ...
  // Filter state
  const [filterMode, setFilterMode] = useState('this-month'); // 'this-month' | 'last-month' | 'custom'
  ...
  const stats = useMemo(() => {
    ...
    let days = 1;
    if (filterMode === 'this-month' || filterMode === 'last-month') {
      days =
        filteredExpenses.length > 0
          ? (new Date(filteredExpenses[0].date) -
              new Date(filteredExpenses.at(-1).date)) /
              (1000 * 60 * 60 * 24) +
            1
          : 1;
    } else if (filterMode === 'custom' && customStart && customEnd) {
  ...
  }, [filteredExpenses, filterMode, customStart, customEnd]);
  ...
  return (
    ...
    >
      ...
      >
        <h3>{editingId ? 'Chỉnh sửa chi tiêu' : 'Thêm chi tiêu mới'}</h3>
        ...
        <div style={{ marginTop: 16, display: 'flex', gap: 12 }}>
          ...
          >
            {editingId ? 'Lưu thay đổi' : 'Thêm chi tiêu'}
          </button>
          {editingId && (
            <button
              onClick={cancelEdit}
      ...
      <section style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          {['this-month', 'last-month', 'custom'].map((mode) => (
            <button
              key={mode}
              ...
              style={{
                ...
                background: filterMode === mode ? '#0d6efd' : '#e9ecef',
                color: filterMode === mode ? 'white' : 'black',
              }}
            >
              {mode === 'this-month'
                ? 'Tháng này'
      ...
      >
        ...
        <div style={{ marginBottom: 12 }}>
          Trung bình/ngày:{' '}
          <strong>
            {stats.averagePerDay.toLocaleString('vi-VN', {
        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {Object.entries(stats.byCategory).map(([cat, amt]) => (
            ...
            >
              {cat}: <strong>{amt.toLocaleString('vi-VN')} ₫</strong>
            </li>
          ))}
        </ul>
      </section>
      ...
      {/* EXPENSE LIST */}
      <section>
        <h3>Danh sách chi tiêu ({filteredExpenses.length})</h3>
              ...
              >
                ...
                >
                  <div>
                    ...
                    <span style={{ marginLeft: 12, color: '#495057' }}>
                      {exp.description || '(không có mô tả)'}
                    </span>
                  </div>
                  <div style={{ display: 'flex', gap: 12 }}>
                    <button onClick={() => startEdit(exp)}>Sửa</button>
                    <button
