...
<details>
<summary>💡 useState Solution</summary>
...
function createInitialMetrics(componentName) {
  return {
    ...
    /**
     * Internal state.
     * Không render trực tiếp field này ra UI nếu không cần.
     */
    samples: [],
  };
}
...
function usePerformance(componentName, options = {}) {
  ...
  const handleRender = useCallback(
    (id, phase, actualDuration) => {
      ...
      setMetrics((prevMetrics) => {
        const nextSamples = [...prevMetrics.samples, renderTime];
        ...
        const nextRenderCount = prevMetrics.renderCount + 1;
        const nextSlowRenderCount =
          prevMetrics.slowRenderCount + (isSlow ? 1 : 0);

        const nextMaxRenderTime = Math.max(
          prevMetrics.maxRenderTime,
          renderTime,
        );

        const nextAverageRenderTime = calculateAverage(nextSamples);

        return {
          componentName: id,
          renderCount: nextRenderCount,
          lastRenderTime: renderTime,
          averageRenderTime: nextAverageRenderTime,
          maxRenderTime: nextMaxRenderTime,
  ...
  const reset = useCallback(() => {
    setMetrics(createInitialMetrics(componentName));
  }, [componentName]);
...
/* =========================================================
   Important: Memoized Profiled Area
========================================================= */

/**
 * Vì usePerformance dùng state nội bộ, mỗi lần metrics update
 * PerformanceMonitor sẽ re-render.
 *
 * Nếu <Profiler> cũng re-render theo update của metrics,
 * rất dễ quay lại vòng lặp:
 *
 * Profiler onRender
 * → setMetrics
 * → PerformanceMonitor re-render
 * → Profiler onRender
 * → setMetrics
 *
 * Vì vậy ta tách vùng được profile ra component memo.
 * Khi metrics update, children/profilerProps không đổi,
 * ProfiledArea sẽ không re-render.
 */
const ProfiledArea = memo(function ProfiledArea({ profilerProps, children }) {
  return <Profiler {...profilerProps}>{children}</Profiler>;
});
...
export function PerformanceMonitor({
  ...
  const { metrics, profilerProps, reset } = usePerformance(name, {
    enabled,
    slowThreshold,
    maxSamples,
  });
  ...
  return (
    <div style={{ position: 'relative' }}>
      <ProfiledArea profilerProps={profilerProps}>{children}</ProfiledArea>
...
function PerformanceOverlay({ metrics, position, slowThreshold, onReset }) {
  const {
    componentName,
    renderCount,
    lastRenderTime,
...
<details>
<summary>💡 useSyncExternalStore Solution</summary>
...
/* =========================================================
 * Singleton store.
 * Một file/module chỉ có một performance store.
========================================================= */
...
function usePerformance(componentName, options = {}) {
  ...
  const handleRender = useCallback(
    ...
    [componentName, enabled, slowThreshold, maxSamples],
  );
...
export function PerformanceMonitor({
  ...
  const { profilerProps } = usePerformance(name, {
    enabled,
    slowThreshold,
    maxSamples,
  });
  ...
  return (
    <div style={{ position: 'relative' }}>
      <Profiler {...profilerProps}>{children}</Profiler>
