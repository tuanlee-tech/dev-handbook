...
Ở bài này, solution tốt nhất là:

```txt
usePerformance()
→ quản lý metrics + expose onRender callback

PerformanceMonitor
→ bọc children bằng React.Profiler
→ truyền actualDuration vào usePerformance
→ hiển thị overlay indicator
```
...
```jsx
const { metrics, reset, enable, disable, profilerProps } = usePerformance(
  componentName,
  options,
);
```
...
```jsx
<PerformanceMonitor name='ProductList'>
  <ProductList />
</PerformanceMonitor>
```
...
```jsx
const perf = usePerformance('ProductList');

return (
  <React.Profiler {...perf.profilerProps}>
    <ProductList />
  </React.Profiler>
);
```
...
```txt
renderCount          số lần render
lastRenderTime       thời gian render gần nhất
averageRenderTime    thời gian render trung bình
maxRenderTime        lần render chậm nhất
slowRenderCount      số lần render > threshold
isSlow               lần render hiện tại có chậm không
phase                mount hoặc update
```
...
Vì 16ms tương đương khoảng 60 FPS. Nếu một render vượt quá 16ms, UI có nguy cơ bị giật.

---

**3. Visualization Strategy**

Dùng overlay nhỏ ở góc component:
...
Need to monitor render performance across app. Developer cần biết component nào render nhiều, render chậm, average render time bao nhiêu, và khi nào vượt ngưỡng 16ms.

**Decision**

Build custom `usePerformance` hook kết hợp với `React.Profiler`, sau đó tạo `PerformanceMonitor` reusable component để wrap quanh subtree cần đo.

**Rationale**

`React.Profiler` cung cấp `actualDuration`, đáng tin hơn việc tự đo bằng `performance.now()` trong render/effect. Hook giúp reusable, còn `PerformanceMonitor` giúp non-invasive vì chỉ cần bọc component.

**Consequences**

Có một lượng overhead nhỏ trong development vì mỗi render sẽ gọi `onRender`. Do đó nên DEV-only, không bật trong production.

**Alternatives**

```txt
React DevTools Profiler
→ Chính xác, mạnh, nhưng manual, không tiện để monitor inline trong UI.

console.log
→ Dễ làm nhưng UX kém, khó aggregate, khó nhìn tổng quan.

performance.now()
→ Có thể dùng cho custom measure, nhưng không nên dùng để đo render duration trực tiếp trong render.
```
...
export function usePerformance(componentName, options = {}) {
  ...
  /**
   * Ref dùng để lưu internal mutable data.
   * Không dùng ref trực tiếp để render UI.
   * Khi cần update UI, ta setMetricsSnapshot.
   */
  ...
  const [metricsSnapshot, setMetricsSnapshot] = useState(() =>
    createInitialMetrics(componentName),
  );
  ...
  const handleRender = useCallback(
    (id, phase, actualDuration, baseDuration, startTime, commitTime) => {
      ...
      const averageRenderTime = calculateAverage(samplesRef.current);

      setMetricsSnapshot({
        componentName: id,
        renderCount: totalRenderCountRef.current,
    ...
    [enabled, slowThreshold, maxSamples],
  );

  const reset = useCallback(() => {
    samplesRef.current = [];
    totalRenderCountRef.current = 0;
    slowRenderCountRef.current = 0;
    maxRenderTimeRef.current = 0;

    setMetricsSnapshot(createInitialMetrics(componentName));
  }, [componentName]);
...
export function PerformanceMonitor({
  ...
  /**
   * Production hoặc disabled thì return children trực tiếp.
   * Không wrap Profiler để tránh production bundle/runtime impact.
   */
  if (!enabled) {
    return <>{children}</>;
  }
  ...
  return (
    <Profiler {...performance.profilerProps}>
...
function PerformanceOverlay({ metrics, onReset, position, slowThreshold }) {
  const {
    componentName,
    renderCount,
    lastRenderTime,
...
```txt
Render time: > 16ms
Possibly > 50ms depending on machine
Indicator: 🔴 red
Slow count increases
```
...
Nếu `NODE_ENV === 'production'`, `PerformanceMonitor` trả thẳng children:
...
Tức là không wrap `Profiler`, không overlay, không measurement.
...
```txt
[x] Test với fast component < 5ms
[x] Test với slow component > 50ms
[x] Track render count
[x] Track last render time
[x] Track average render time
[x] Track max render time
[x] Detect slow render > 16ms
[x] Visual indicator xanh/đỏ
[x] Reset metrics
[x] Không chạy trong production nếu enabled = false
```
...
Với bài 60 phút, implementation trên là hợp lý. Nhưng nếu scale lên toàn app, không nên để mỗi monitor tự `setState` quá nhiều, vì chính monitor có thể gây thêm render. Khi đó nên nâng cấp thành:

```txt
React.Profiler
→ external performance store
→ useSyncExternalStore
→ debounced dashboard
```

Bài này yêu cầu reusable hook + visual indicator, nên `usePerformance + PerformanceMonitor` là đủ gọn, đúng lifecycle, dễ dùng.
