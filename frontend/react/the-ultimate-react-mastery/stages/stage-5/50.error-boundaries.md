  ...
  async retryWithBackoff(fn, maxRetries = 3, baseDelayMs = 1000) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.log(
          `🔄 Retry ${attempt}/${maxRetries - 1} failed, waiting ${delay}ms...`,
        );
        ...
        return (
          ...
          >
            <div>
              ...
              <div
                style={{ color: style.color, fontSize: 13, lineHeight: 1.5 }}
              >
...
function WidgetErrorFallback({
  ...
  return (
    ...
    >
      ...
      <h3
        style={{ margin: '0 0 8px', color: hasCache ? '#92400e' : '#dc2626' }}
      >
...
function ComponentErrorFallback({ error, resetErrorBoundary }) {
  return (
    ...
    >
      <span style={{ fontWeight: 600 }}>⚠️ Component error:</span>{' '}
      {error.message}{' '}
      <button
        onClick={resetErrorBoundary}
        style={{
  ...
  return (
    ...
    >
      ...
      >
        ...
        <StatCard
          label='App Level'
          value={stats.byLevel.app || 0}
          color='#7c3aed'
          bg='#ede9fe'
        />
        <StatCard
          label='Widget Level'
          value={stats.byLevel.widget || 0}
          color='#2563eb'
          bg='#dbeafe'
        />
        <StatCard
          label='Cached Recovery'
          value={stats.byComponent?.cache || 0}
          color='#059669'
          bg='#d1fae5'
        />
      </div>
            ...
            {errors
              ...
              .map((err) => (
                ...
                >
                  ...
                  <td style={tableCellStyle}>
                    {err.context?.component || 'Unknown'}
                  </td>
                  <td style={tableCellStyle}>
                    <Badge text={err.context?.level || 'unknown'} />
                  </td>
                  <td style={tableCellStyle}>
                    <PriorityBadgeAnalytics
                      text={err.context?.priority || 'unknown'}
                    />
                  </td>
                  <td
                    style={{
    ...
    <div style={{ backgroundColor: '#f3f4f6', minHeight: '100vh' }}>
      ...
      >
        ...
        <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ marginBottom: 24 }}>
            <h1 style={{ margin: 0, fontSize: 24, color: '#111827' }}>
              Dashboard Overview
            </h1>
          ...
          >
            ...
            <div style={{ fontSize: 13, lineHeight: 1.8, color: '#4b5563' }}>
              <p>
                <strong>Level 1 (App):</strong> Catches catastrophic errors →
                Full page error UI
              </p>
              <p>
                <strong>Level 2 (Feature):</strong> Navigation isolation →
                Minimal fallback nav
              </p>
              <p>
                <strong>Level 3 (Widget):</strong> Per-widget isolation → Error
                + retry per card
              </p>
              <p>
                <strong>Level 4 (Component):</strong> Small component wrap →
                Inline error
              </p>
              ...
              >
                ...
                <ul style={{ margin: '8px 0 0', paddingLeft: 20 }}>
                  ...
                  <li>
                    Silent failure: Non-critical components hide gracefully
                  </li>
                </ul>
              </div>
            </div>
...
### 3 CHIẾN LƯỢC PHỤC HỒI LỖI CHÍNH

**1. Retry với Exponential Backback trong Error Boundary**
...
class RetryErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.isRetrying) {
      return (
        this.props.fallbackRetrying || (
          <div>Đang thử lại sau {Math.pow(2, this.state.retryCount)}s...</div>
        )
      );
    }
    ...
    if (this.state.hasError) {
      return (
        this.props.fallback || (
          <div>
            <p>
              Lỗi sau {this.state.retryCount} lần thử:{' '}
              {this.state.error?.message}
            </p>
            <button
              onClick={() => {
...
**2. Fallback sang Cached Data trong Error Boundary**

```jsx
class CacheFallbackBoundary extends React.Component {
  constructor(props) {
    super(props);
    this.state = { hasError: false, cachedData: null };
  ...
  componentDidCatch(error, errorInfo) {
    // Lấy cache từ localStorage/sessionStorage
    try {
      const cached = localStorage.getItem(this.cacheKey);
      if (cached) {
        this.setState({ cachedData: JSON.parse(cached) });
      }
    } catch (e) {
  ...
  // Component con gọi để lưu cache
  ...
  render() {
    if (this.state.hasError && this.state.cachedData) {
      return (
        <div>
          <div className='cache-banner'>
            ⚠️ Hiển thị dữ liệu đã lưu (
            {new Date(this.state.cachedData.timestamp).toLocaleString()})
          </div>
          {/* Clone children và truyền cachedData qua context hoặc prop */}
          {React.cloneElement(this.props.children, {
            cachedData: this.state.cachedData.data,
          })}
        </div>
      );
    }
    ...
    if (this.state.hasError) {
      return (
        this.props.fallback || <div>Không có dữ liệu cache để hiển thị</div>
      );
    }
...
**3. Degraded Mode trong Error Boundary**

```jsx
class DegradedModeBoundary extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      hasError: false,
  ...
  componentDidCatch(error, errorInfo) {
    // Xác định component nào lỗi dựa trên errorInfo
    const componentStack = errorInfo.componentStack;

    // Map lỗi component -> degraded version
    const degradedMap = this.props.degradedComponents || {};

    for (const [componentName, degradedConfig] of Object.entries(degradedMap)) {
      if (componentStack.includes(componentName)) {
        this.setState({
          failedComponent: componentName,
          degradedProps: degradedConfig.props || {},
        });
        break;
      }
    }
  ...
  render() {
    ...
    const { failedComponent, degradedProps } = this.state;
    const degradedMap = this.props.degradedComponents || {};

    if (failedComponent && degradedMap[failedComponent]) {
      const DegradedComponent = degradedMap[failedComponent].component;

      return (
        <div className='degraded-container'>
          <div className='degraded-banner'>
            ⚠️ Một số tính năng không khả dụng. Hiển thị giao diện đơn giản.
          </div>
          <DegradedComponent {...degradedProps} />
        </div>
      );
    }

    // Không có degraded component, hiển thị fallback thông thường
    return this.props.fallback || <div>Có lỗi xảy ra</div>;
  }
}
...
function Dashboard() {
  return (
    <DegradedModeBoundary
      degradedComponents={{
        ComplexChart: {
          component: SimpleChart,
          props: { data: [] }, // Dữ liệu đơn giản hơn
        },
        LiveMap: {
          component: StaticMap,
          props: { imageUrl: '/fallback-map.png' },
        },
      }}
      fallback={<div>Dashboard không khả dụng</div>}
    >
      <ComplexChart data={heavyData} />
      <LiveMap coordinates={coords} />
    </DegradedModeBoundary>
  );
}
```

**Kết hợp cả 3: ResilientErrorBoundary hoàn chỉnh**
...
class ResilientErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    const { maxRetries = 3, cacheKey, degradedConfig } = this.props;
    ...
    // 2. Hết retry -> thử Cache
    if (cacheKey) {
      const cached = this.tryLoadCache(cacheKey);
      if (cached) {
        this.setState({ usingCache: true, cachedData: cached });
        return;
      }
    }

    // 3. Không có cache -> Degraded mode
    if (degradedConfig) {
      this.setState({ usingDegraded: true });
    }
  }

  attemptRetry = () => {
    const delay =
      Math.pow(2, this.state.retryCount) * 1000 + Math.random() * 1000; // + jitter
  ...
  tryLoadCache = (key) => {
    try {
      ...
      const isExpired =
        Date.now() - timestamp > (this.props.cacheDuration || 5 * 60 * 1000);
  ...
  render() {
    ...
    const { degradedComponent: DegradedComponent, children } = this.props;

    // Đang retry
    if (isRetrying) {
      return (
        <div className='retrying'>
          <p>Đang thử lại... (lần {retryCount + 1})</p>
          <progress />
        </div>
      );
    }
      ...
      return (
        <div>
          <div className='cache-notice'>
            ⚠️ Hiển thị dữ liệu đã lưu.
            <button onClick={this.handleManualRetry}>Cập nhật</button>
          </div>
          {React.cloneElement(children, { cachedData })}
        </div>
      );
    }

    // Degraded mode
    if (usingDegraded && DegradedComponent) {
      return (
        <div>
          <div className='degraded-notice'>⚠️ Một số tính năng bị giới hạn</div>
          <DegradedComponent />
        </div>
      );
    }
    ...
    return (
      <div className='error-fallback'>
        ...
        <p>{error?.message}</p>
        <button onClick={this.handleManualRetry}>Thử lại</button>
      </div>
    );
  }
}
...
// Component con báo lỗi
function DataComponent({ cachedData }) {
  // Dùng cachedData nếu có, hoặc fetch mới
  const data = cachedData || useFetchData();
  return <div>{data}</div>;
}
...
function App() {
  return (
    <ResilientErrorBoundary
      ...
      degradedComponent={SimpleDashboard}
    >
      <DataComponent />
    </ResilientErrorBoundary>
  );
...
| Vấn đề                                             | Giải pháp                                                              |
| -------------------------------------------------- | ---------------------------------------------------------------------- |
| Error Boundary chỉ bắt lỗi render, không bắt async | Dùng try-catch trong `useEffect` + `setState` để trigger re-render lỗi |
| Infinite retry loop                                | Giới hạn `maxRetries`, reset count khi props thay đổi                  |
| Cache stale                                        | Thêm `timestamp` kiểm tra, hoặc dùng versioned cache                   |
| Degraded phức tạp                                  | Tách thành nested boundaries: critical + non-critical riêng            |

```jsx
// Pattern: Nested boundaries cho partial degradation
<RetryErrorBoundary maxRetries={2}>
  {' '}
  {/* Critical: retry ngay */}
  <UserProfile />
  <CacheFallbackBoundary cacheKey='stats'>
    {' '}
    {/* Non-critical: cache fallback */}
    <UserStats />
  </CacheFallbackBoundary>
  <DegradedModeBoundary degradedComponent={SimpleChart}>
    {' '}
    {/* Optional: degraded */}
    <ComplexChart />
  </DegradedModeBoundary>
